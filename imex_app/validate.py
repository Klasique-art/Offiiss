...
def validate_code(request):
    ...
    if code:
        if datetime.now(tz=pytz.utc) < code.expiring_date:
            ...
            user.save()
            send_mail('Offiss Welcome message', f'Hello {code.user.first_name}, Your offiiss account has been validated \n\n\n Need help? Send an email to our support team at support@offiis.com.', 'offiissapp@offiiss.com',  [code.user.email], fail_silently=True)
            return Response({"status": "ok"})
            code.delete()
